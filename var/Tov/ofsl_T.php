...
<input type="hidden" name="viewInfo" value="Товарный_Официальный"/>
...
<p>Официальный коэффициент вариации</p>
 ...
 while($row_2 = $result_2->fetch_array()){ //этот цикл вычисляет сумму квадратов 
  ...
  <tr>
   ...
   <td align="center"><?php echo str_replace('.',',',number_format(round($mid_s,1), 1, '.', ''))?></td>
   <td align="center"></td>
   <td align="center"><?php echo str_replace('.',',',number_format(round($sumR,1), 1, '.', ''))?> </td>
   <td align="center"><?php echo str_replace('.',',',number_format(($P_max-$P_min), 1, '.', ''))?></td>   
...
Rmin = <?=$Rt-4?> МПа<br/>
В&ltRmini≥Rmin	<?=$matches[2]?>&lt<?=$P_min?>&gt<?=$Rt-4?><br/>
...
<p align=center >------------------------------------------------------------------------------------------------------------------------</p>
 ...
 <table border="1px" align=center bgcolor=#eaeaea cellpadding="4px" cellspacing="0px" id="table3">
	<caption>Результаты статистического метода контроля прочности товарного бетона по ГОСТ 18105-2010. Период с <?=$data1?> по <?=$data2?>.    </caption>
  <tr>
